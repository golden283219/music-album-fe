...
export default function AlbumPage() {
  ...
  let index = 0;
  console.log(album)
  const categoryCount = album?.category.length || 0;
  for (let i = 0; i < categoryCount; i++) {
    genres.push(<NavLink to={`/genres/${album?.category[i].slug}/s/LIST/p/0`} key={index++}
                         className="genre-link">{album?.category[i].name}</NavLink>);
